...
X_test = np.clip(scaler.transform(X_test), h, 1 - h)

y_train = y_train.reshape(-1)
y_test = y_test.reshape(-1)

input_feature_names = breast.data.features.columns.tolist()
output_names = breast.data.targets.columns.tolist()
...
def create_new_qbaf(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    new_qbaf = copy.deepcopy(original_qbaf)

    for relation, weight in new_weights.items():
        initial = new_qbaf.arguments[relation[0]]
        final = new_qbaf.arguments[relation[1]]
...
for example_num in tqdm(range(len(X_test)), desc="Example Num"):
    ...
    original_ce = original_ce.get_ce_explanation()

    _, original_distance = check_validity(
        original_qbaf,
        desired_strength,
        delta,
        output_names[0],
    ...
    avg_weights = np.mean(list(original_ce.values()))
    ...
    for i, sparsification in enumerate(sparsification_amounts):
        ...
        sparse_ce = sparse_ce.get_ce_explanation()

        avg_weights_sparse = np.mean(list(sparse_ce.values()))

        diff = abs(avg_weights - avg_weights_sparse)

        approx_cf_qbaf = create_new_qbaf(
            original_qbaf,
            sparse_qbaf,
            sp,
            sparse_ce,
        ...
        valid, approximation_distance = check_validity(
            approx_cf_qbaf,
            desired_strength,
            delta,
            output_names[0],
        ...
        differences[example_num, i] = diff
        valids[example_num, i] = valid
        distances[example_num, i] = approximation_distance
        original_distances[example_num, i] = original_distance


size_str = "_".join([str(s) for s in size])

np.savez_compressed(
    f"outputs/cancer/ce/mlp_{size_str}.npz",
